...
def verify_and_confirm(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request,
        f"{count} order(s) confirmed and customer notified."
    )
...
def mark_shipped(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request,
        f"{count} order(s) marked shipped — customer notified by email + SMS."
    )
...
def mark_delivered(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request,
        f"{count} order(s) marked delivered — customer notified."
    )
    ...
    def screenshot_preview(self, obj):
        if obj.payment_screenshot:
            return format_html(
                ...
                '<img src="{0}" style="height:50px;border-radius:6px;" />'
                '</a>',
    ...
    def engraving_summary(self, obj):
        ...
        for item in obj.items.all():
            ...
            if item.custom_name:
                ...
                parts.append(
                    f"{label} ({item.product_name}, x{item.quantity})"
                )
